...
function [Lfinal,STATSfinal] = mask2ellipses(L,Img,mattype,SolidityThresh,minRegionSize)
...
if nargin < 3, covar_type = 'full'; end

if nargin < 4, SolidityThresh = 0.98; end

if nargin < 5,     
%     minRegionSize = 30; 
    minRegionSize = floor(numel(find(L))/4);
end
...
init_opts = foptions; opts(1) = 0; opts(14) = 10; % for the kmean
for i=1:N,
...
%             X = X - repmat(mean(X),size(X,1),1);
            ...
            if numel(Img) > 1,
                ...
                X = [X,tmp - min(tmp)];
                mix_std0 = gmm(3, K, covar_type); % 'full'!!! create the EM object with two centers
            else
