...
public:
        ...
        int Grab(AVbinPacket* packet)
        {
                ...
                if (isAudio)
                {
                        ...
                        if (DEBUG) FFprintf("Push audiobuf %ld: %d bytes to %x\n", timestamp, len, tmp);
                        ...
                        if (DEBUG) FFprintf("avbin_decode_video %d to %x\n", packet->size, videobuf);

                        int nBytesRead = avbin_decode_video(stream, packet->data, packet->size, videobuf);
                        if (DEBUG) FFprintf("avbin_decode_video read %d to %x which can hold %d\n", nBytesRead, videobuf, bytesPerWORD);
                        if (nBytesRead<=0)
                        {
                        ...
                        if (skip || len==0)
                        {
                                if (DEBUG) FFprintf("Free videobuf %x\n", videobuf);
                                av_free(videobuf);
                                return 0;
                        } else {
                                size_t buflen = min(len, bytesPerWORD);
                                if (DEBUG) FFprintf("Push videobuf %f: %d bytes to %x\n", timestamp, buflen, videobuf);
...
int FFGrabber::build(const char* filename, char* format, bool disableVideo, bool disableAudio, bool tryseeking)
{
        ...
        if (!file) {
                if (DEBUG) FFprintf("Error avbin_open_filename %s\n", filename);
                return -4;
        }
        ...
        if (avbin_file_info(file, &fileinfo)) {
                if (DEBUG) FFprintf("ERROR avbin_file_info\n");
                return -1;
        }
        if (DEBUG) FFprintf("avbin_file_info size: %ld\n", fileinfo.structure_size);
        ...
        for (int stream_index=0; stream_index<fileinfo.n_streams; stream_index++)
        {
                ...
                {
                        AVRational frame_rate = file->context->streams[stream_index]->avg_frame_rate;
