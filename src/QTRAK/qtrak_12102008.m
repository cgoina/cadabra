...
global nframes_mean mean_image std_image frmindx;
    ...
    if ~params.bool_asked,
        ...
        %% 
        % Finally, create the menu and submenu items along with specifying
    ...
    strInVideoFNameArray = 'media\EH080525Pw1C2.wmv';
    Files.strInVideoPath = [ pwd '\' ];
    params.findex = 1;
    openavi(1);
    params.filechg = 0;
...
%             randframes = intStartFrm:intStartFrm+nframes_mean-1;

            try
                mmread( Files.strInFName,randframes, ... 
                        [],false,false,'ProcessFrameMeanWin',false);
%                 mmread( Files.strInFName,intStartFrm : intStartFrm+nframes_mean, ... 
%                         [],false,false,'ProcessFrameMeanWin',false);
            catch err
        ...
        try
            mmread( Files.strInFName, intStartFrm-1 : intStartFrm+intNFrms, ... 
                    [], false, false, 'ProcessFrameCapWin', false );
        catch err
            if ~strcmp(err.message(end-14:end),'STOP PROCESSING')
                rethrow(err); 
            end
