...
function textoutput(data,gen_ident,titl,action_name,params,xtitl,xvals)
...
for igen=1:ngens,
    for ifly=1:size(data,2),
        if size(data,2) > 1,
            if params.winlos,
                if ifly == 1, flynam = 'winner'; else flynam = 'loser '; end
            else
                flynam = ['fly ' num2str(ifly)]; 
            end
            fprintf(FID,['%' num2str(maxlen) 's %5s'],cell2mat(gen_ident(igen)),flynam);
        else
