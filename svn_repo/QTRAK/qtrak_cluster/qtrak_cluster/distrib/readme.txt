    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying standalone applications, it is possible 
              to run the shell script file run_qtrak_cluster.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    NOTE: qtrak_cluster.ctf will be present only if you have chosen 
    ...
    All Platforms:
        * qtrak_cluster.ctf (if present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * qtrak_cluster.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * qtrak_cluster (application created by MATLAB Compiler)
        * run_qtrak_cluster.sh (shell script that can be run to 
    ...
    All Platforms:
        ...
        * qtrak_cluster.so (shared library, file extension varies by 
          platform)
        * qtrak_cluster.h (library header file)
    ...
    Windows:
        * qtrak_cluster.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)    
        * qtrak_cluster.xml (documentation files)
        * qtrak_cluster.pdb (program database file - if DEBUG option 
          selected)
        * qtrak_cluster.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * qtrak_cluster.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * qtrak_cluster_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * qtrak_cluster.jar (Java class files and compiled M-code)
    ...
    Windows:
        * qtrak_cluster.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * qtrak_cluster_1_0.dll (COM component)
        * qtrak_cluster.xla (optionally created and placed in
          /home/liujin/EricHoopfer/trunk/QTRAK/qtrak_cluster/qtrak_cluster/src directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
