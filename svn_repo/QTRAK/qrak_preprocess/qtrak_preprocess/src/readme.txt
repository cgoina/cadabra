    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying standalone applications, it is possible 
              to run the shell script file run_qtrak_preprocess.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    NOTE: qtrak_preprocess.ctf will be present only if you have chosen 
    ...
    All Platforms:
        * qtrak_preprocess.ctf (if present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * qtrak_preprocess.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * qtrak_preprocess (application created by MATLAB Compiler)
        * run_qtrak_preprocess.sh (shell script that can be run to 
    ...
    All Platforms:
        ...
        * qtrak_preprocess.so (shared library, file extension varies by 
          platform)
        * qtrak_preprocess.h (library header file)
    ...
    Windows:
        * qtrak_preprocess.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)    
        * qtrak_preprocess.xml (documentation files)
        * qtrak_preprocess.pdb (program database file - if DEBUG option 
          selected)
        * qtrak_preprocess.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * qtrak_preprocess.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * qtrak_preprocess_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * qtrak_preprocess.jar (Java class files and compiled M-code)
    ...
    Windows:
        * qtrak_preprocess.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * qtrak_preprocess_1_0.dll (COM component)
        * qtrak_preprocess.xla (optionally created and placed in
          /home/liujin/SourceCode/compiledqtrak/qtrak_preprocess/src directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
